...
let rec step (inst : instance) (c : config) : config =
  ...
  let vs', es' =
    ...
    | Plain e', vs ->
      ...
      | BrTable (xs, x), Num (I32 i) :: vs' when I32.ge_u i (Lib.List32.length xs) ->
        vs', [Plain (Br x) @@ e.at]
      ...
      | GetField (x, y), Ref (Data (Struct s)) :: vs' ->
        (try
      ...
      | Load {offset; ty; sz; _}, Num (I32 i) :: vs' ->
        ...
        (try
          let n =
            match sz with
            | None -> Num (Memory.load mem addr offset ty)
            | Some (sz, ext) -> Num (Memory.load_packed sz ext mem addr offset ty)
          in (n :> value) :: vs', []
      ...
      | Test testop, v :: vs' ->
        (try Num (num_of_bool (Eval_numeric.eval_testop testop v)) :: vs', []
        with exn -> vs', [Trapped (numeric_error e.at exn) @@ e.at])

      | Compare relop, v2 :: v1 :: vs' ->
        (try Num (num_of_bool (Eval_numeric.eval_relop relop v1 v2)) :: vs', []
        with exn -> vs', [Trapped (numeric_error e.at exn) @@ e.at])
